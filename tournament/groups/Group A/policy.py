...
def _infer_me(b: np.ndarray) -> int:
    # act() no recibe el color: se infiere por paridad de fichas.
    # par -> mueve Rojo(-1); impar -> Amarillo(1).
...
def _wins_at(b: np.ndarray, r: int, c: int, p: int) -> bool:
    # ¿La ficha p recién colocada en (r,c) completa 4 en línea?
...
def _winner(b: np.ndarray) -> int:
    # Escaneo completo (referencia; usado por el harness y assert de equivalencia
    # con ConnectState.get_winner).
    for r in range(6):
        for c in range(7):
            p = b[r, c]
            if p == 0:
...
def _minimax(b, depth, alpha, beta, maximizing, me, lr, lc, lp):
    # lp = jugador que acaba de mover en (lr,lc); si ganó, el estado es terminal.
...
class MinimaxPolicy(Policy):
    ...
    def act(self, s: np.ndarray) -> int:
        ...
        for c in legal:
            ...
            if block:
                return c
        # Capa 3: minimax con poda alfa-beta
